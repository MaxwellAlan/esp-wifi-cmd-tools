...
extern wifi_socket_t socket_args;
...
void register_wifi(void)
{
    ...
    // Socekt command
    socket_args.ip = arg_str0("i", "ip", "<ip>", "peer ip addr");
    socket_args.port = arg_int0("p", "port", "<port>", "port to listen on/connect to");
    socket_args.length = arg_int0("l", "len", "<length>", "Set read/write buffer size");
    socket_args.cnt = arg_int0("n", "cnt", "<count>", "packet counts to send");
    socket_args.interval = arg_int0("j", "interval", "<interval>", "send packet interval");
    socket_args.bind = arg_lit0("B", NULL, "socket bind");
    socket_args.send = arg_lit0("S", NULL, "socket send");
    socket_args.type = arg_str0("t", "type", "<type>", "UDP or TCP");
    socket_args.socket = arg_int0("s", NULL, "<socket>", "socket number");
    socket_args.end = arg_end(1);
    const esp_console_cmd_t socket_cmd = {
        .command = "socket",
        .help = "socket command",
        .hint = NULL,
        .func = &wifi_cmd_socket,
        .argtable = &socket_args
    };
    ESP_ERROR_CHECK( esp_console_cmd_register(&socket_cmd) );
